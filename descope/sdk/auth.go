...
type EnchantedLink interface {
	...
	// SignInWithPhone - Use to login a user based on an enchanted link that will be sent by SMS to the given phone.
	// the jwt would be returned on the getSession function at the end of the flow rather that on the verify.
	// returns the masked phone to which the link was sent, or an error upon failure.
	SignInWithPhone(ctx context.Context, phone, URI string, r *http.Request, loginOptions *descope.LoginOptions) (*descope.PhoneEnchantedLinkResponse, error)
	...
	// SignUpWithPhone - Use to create a new user based on the given phone, verified by an enchanted link sent by SMS.
	// optional to add user metadata for farther user details such as name and more.
	// returns the masked phone to which the link was sent, or an error upon failure.
	SignUpWithPhone(ctx context.Context, phone, URI string, user *descope.User, signUpOptions *descope.SignUpOptions) (*descope.PhoneEnchantedLinkResponse, error)
	...
	// with the given phone. The enchanted link is sent by SMS.
	// optional to add user metadata for farther user details such as name and more.
	// returns the masked phone to which the link was sent, or an error upon failure.
	SignUpOrInWithPhone(ctx context.Context, phone string, URI string, signUpOptions *descope.SignUpOptions) (*descope.PhoneEnchantedLinkResponse, error)
	...
	// UpdateUserPhone - Use to update phone and validate via an enchanted link sent by SMS.
	// LoginID of user whom we want to update
	// UpdateOptions to determine whether to add phone as a login id and if to merge with existing user in that case
	// Request is needed to obtain JWT and send it to Descope, for verification
	UpdateUserPhone(ctx context.Context, loginID, phone, URI string, updateOptions *descope.UpdateOptions, request *http.Request) (*descope.PhoneEnchantedLinkResponse, error)
}
