...
type Project interface {
	...
	Clone(ctx context.Context, name string, environment descope.ProjectEnvironment, tags []string) (*descope.CloneProjectResponse, error)
...
type ManagementKey interface {
	...
	// CreateWithOptions creates a new management key from an options struct, which carries every
	// field a key can be created with.
	//
	// The key object is returned along with its token and an optional error. A key that
	// authenticates without a secret has no token, so the returned cleartext is empty for one.
	...
	// UpdateWithOptions updates an existing management key from an options struct, which carries
	// every field an update can set.
	//
