...
type Authentication interface {
	...
	// and choose the selected delivery method for verification (see auth/DeliveryMethod).
	// if crossDevice is true, it will return a pending reference to be used in GetPendingSession, which should get the session once the link was verified.
	// if crossDevice is false, the pending reference will be an empty string, and VerifyMagicLink will return an authenticated session.
	// returns an error upon failure.
	SignInMagicLink(method DeliveryMethod, identifier, URI string, crossDevice bool) (MagicLinkResponse, error)
	...
	// if crossDevice is false, the pending reference will be an empty string, and VerifyMagicLink will return an authenticated session.
	// returns an error upon failure.
	SignUpMagicLink(method DeliveryMethod, identifier, URI string, crossDevice bool, user *User) (MagicLinkResponse, error)
	...
	// This is a shortcut for GetPendingSessionWithOptions(method, code, WithResponseOption(w))
	GetPendingSession(pendingRef string, w http.ResponseWriter) (*AuthenticationInfo, error)
	// GetPendingSessionWithOptions - Use to get a session that was generated by SignInMagicLink/SignUpMagicLink request, and verified with VerifyMagicLink request.
	...
	// if the link was generated with crossDevice, the authentication info will be nil, and should returned with GetPendingSession.
