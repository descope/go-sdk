...
type MagicLink interface {
	...
	SignIn(method DeliveryMethod, loginID, URI string, r *http.Request, loginOptions *LoginOptions) error
	...
	// returns an error upon failure.
	SignUp(method DeliveryMethod, loginID, URI string, user *User) error
	...
	// optional to add user metadata for farther user details such as name and more.
	// returns an error upon failure.
	SignUpOrIn(method DeliveryMethod, loginID string, URI string) error
	...
	// LoginID of user whom we want to update
	// Request is needed to obtain JWT and send it to Descope, for verification
	UpdateUserPhone(method DeliveryMethod, loginID, phone, URI string, request *http.Request) error
}
...
type EnchantedLink interface {
	...
	// returns an error upon failure.
	SignIn(loginID, URI string, r *http.Request, loginOptions *LoginOptions) (*EnchantedLinkResponse, error)

	// SignUp - Use to create a new user based on the given loginID either email or a phone.
	// optional to add user metadata for farther user details such as name and more.
	// returns an error upon failure.
	SignUp(loginID, URI string, user *User) (*EnchantedLinkResponse, error)

	// SignUpOrIn - Use to login in using loginID, if user does not exists, a new user will be created
	// with the given loginID.
	// optional to add user metadata for farther user details such as name and more.
	// returns an error upon failure.
	SignUpOrIn(loginID string, URI string) (*EnchantedLinkResponse, error)
	...
	// LoginID of user whom we want to update
	// Request is needed to obtain JWT and send it to Descope, for verification
	UpdateUserEmail(loginID, email, URI string, request *http.Request) (*EnchantedLinkResponse, error)
}
...
type OTP interface {
	...
	// and choose the selected delivery method for verification. (see auth/DeliveryMethod)
	// returns an error upon failure.
	SignIn(method DeliveryMethod, loginID string, r *http.Request, loginOptions *LoginOptions) error

	// SignUp - Use to create a new user based on the given loginID either email or a phone.
	// choose the selected delivery method for verification. (see auth/DeliveryMethod)
	// optional to add user metadata for farther user details such as name and more.
	// returns an error upon failure.
	SignUp(method DeliveryMethod, loginID string, user *User) error

	// SignUpOrIn - Use to login in using loginID, if user does not exists, a new user will be created
	// with the given loginID.
	SignUpOrIn(method DeliveryMethod, loginID string) error
	...
	VerifyCode(method DeliveryMethod, loginID string, code string, w http.ResponseWriter) (*AuthenticationInfo, error)
	...
	// LoginID of user whom we want to update
	// Request is needed to obtain JWT and send it to Descope, for verification
	UpdateUserPhone(method DeliveryMethod, loginID, phone string, request *http.Request) error
}
...
type TOTP interface {
	...
	SignUp(loginID string, user *User) (*TOTPResponse, error)
	...
	// Use the ResponseWriter (optional) to apply the cookies to the response automatically.
	// returns a list of cookies or an error upon failure.
	SignInCode(loginID string, code string, r *http.Request, loginOptions *LoginOptions, w http.ResponseWriter) (*AuthenticationInfo, error)

	// UpdateUser - set a seed to an existing user, so the user can use an authenticator app
	UpdateUser(loginID string, request *http.Request) (*TOTPResponse, error)
}
...
type WebAuthn interface {
	...
	SignUpStart(loginID string, user *User, origin string) (*WebAuthnTransactionResponse, error)
	...
	SignInStart(loginID string, origin string, r *http.Request, loginOptions *LoginOptions) (*WebAuthnTransactionResponse, error)
	...
	// by calling document.location.origin via javascript.
	// returns a transaction id response on successs and error upon failure.
	SignUpOrInStart(loginID string, origin string) (*WebAuthnTransactionResponse, error)
	...
	// by calling document.location.origin via javascript.
	// returns a transaction id response on success and error upon failure.
	UpdateUserDeviceStart(loginID string, origin string, request *http.Request) (*WebAuthnTransactionResponse, error)
