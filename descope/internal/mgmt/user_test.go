...
func TestUserTOTPSeedSuccess(t *testing.T) {
	response := map[string]any{}
	m := newTestMgmt(nil, helpers.DoOkWithBody(func(r *http.Request) {
		require.Equal(t, r.Header.Get("Authorization"), "Bearer a:key")
		req := map[string]any{}
	...
	err := m.User().RemoveTOTPSeed(context.Background(), "abc")
	require.NoError(t, err)
}

func TestUserTOTPSeedBadInput(t *testing.T) {
	m := newTestMgmt(nil, helpers.DoOk(nil))
	err := m.User().RemoveTOTPSeed(context.Background(), "")
	require.Error(t, err)
}

func TestUserTOTPSeedError(t *testing.T) {
	m := newTestMgmt(nil, helpers.DoBadRequest(nil))
	err := m.User().RemoveTOTPSeed(context.Background(), "abc")
	require.Error(t, err)
}
...
func TestUserProviderTokenWithOptionsSuccess(t *testing.T) {
	...
	assert.EqualValues(t, "pro", res.Provider)
	assert.EqualValues(t, "abc", res.AccessToken)
	assert.EqualValues(t, "def", res.RefreshToken)
}
	...
	response := map[string]any{
		"devices": []any{
			map[string]any{
				...
				"userId":         "abc",
				"deviceType":     "desktop",
				"lastLoginTime":  int64(1724411111),
				"expirationTime": int64(1727411111),
				"lastLocation":   "US",
			},
			map[string]any{
				"id":             "TD2",
				...
				"userId":         "abc",
				"deviceType":     "mobile",
				"lastLoginTime":  int64(1724412222),
				"expirationTime": int64(1727412222),
				"lastLocation":   "IL",
			},
		},
	}
