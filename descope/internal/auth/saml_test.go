...
func TestSAMLStartStepup(t *testing.T) {
	...
	require.NoError(t, err)
	w := httptest.NewRecorder()
	urlStr, err := a.SAML().Start(context.Background(), tenant, landingURL, &http.Request{Header: http.Header{"Cookie": []string{"DSR=test"}}}, &descope.LoginOptions{Stepup: true, CustomClaims: map[string]any{"k1": "v1"}}, w)
	require.NoError(t, err)
	assert.EqualValues(t, uri, urlStr)
	assert.EqualValues(t, urlStr, w.Result().Header.Get(descope.RedirectLocationCookieName))
	assert.EqualValues(t, http.StatusTemporaryRedirect, w.Result().StatusCode)
